"""
scripts/index/reliable_qdrant_uploader.py — Resilient Sequential Qdrant Cloud Ingestion
"""
...
def main():
    print("=" * 82, flush=True)
    print("  VaaniRAG — Resilient Qdrant Cloud Ingestion Pipeline", flush=True)
    print("=" * 82, flush=True)
    print(f"Cluster:    {QDRANT_URL}", flush=True)
    print(f"Dimension:  {EMBEDDING_DIM}-dim (Cosine)", flush=True)
    print(f"Batch Size: {BATCH_SIZE} points/request", flush=True)
    print("-" * 82, flush=True)
    ...
    for strategy in ORDERED_STRATEGIES:
        ...
        print(f"   Loaded {len(all_chunks):,} chunks. Ingesting {total_batches} batches...", flush=True)
        t0 = time.time()
        uploaded_points = 0
        ...
        for b_idx in range(0, len(all_chunks), BATCH_SIZE):
            ...
            for attempt in range(1, 4):
                try:
                    ...
                    if attempt == 3:
                        print(f"   ⚠️ Batch {b_idx // BATCH_SIZE + 1} failed: {e}", flush=True)
                    time.sleep(attempt * 0.5)

            if (b_idx // BATCH_SIZE + 1) % 20 == 0 or (b_idx + BATCH_SIZE) >= len(all_chunks):
                pct = ((b_idx + len(chunk_slice)) / len(all_chunks)) * 100
                print(f"   └─ Ingestion Progress: {uploaded_points:,}/{len(all_chunks):,} ({pct:.1f}%)", flush=True)

            time.sleep(0.02)  # 20ms pacing

        duration = time.time() - t0
        print(f"   ✅ [{strategy.upper()}] Uploaded {uploaded_points:,} vectors in {duration:.2f}s ({uploaded_points / max(1, duration):.0f} pts/sec).", flush=True)
    ...
    print("\n" + "=" * 82, flush=True)
    print("  QDRANT CLUSTER INGESTION COMPLETE", flush=True)
    print("=" * 82, flush=True)
