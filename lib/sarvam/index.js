...
// Minimum blob size to bother sending. Audio smaller than this is almost
// certainly empty/silent and Sarvam will 422 it anyway — detect early.
const MIN_BLOB_BYTES = 1_000; // ~1 KB

/**
 * Transcribe an audio Blob using Sarvam AI Saarika v3.
 *
...
export async function transcribeAudio(audioBlob, options = {}) {
  ...
  // ── Pre-flight: reject clearly empty/silent blobs before spending an API call
  if (!audioBlob || audioBlob.size < MIN_BLOB_BYTES) {
    /** @type {SarvamTranscriptionError} */
    const err = {
      type: 'silent-audio',
      message: `Audio blob is too small (${audioBlob?.size ?? 0} bytes). Recording may be empty or silent.`,
    };
    throw err;
  }
  ...
  // Note: Saarika v3 performs automatic language detection — no need to pass
  // a language hint for detection. languageHint is accepted in options for
  // future use (e.g. to pass to the mode param) but unused for now.
  ...
  // Sarvam v3 transcription response: { request_id, transcript, language_code }
  ...
  if (!text.trim()) {
    ...
    const err = {
      type:    'silent-audio',
      message: 'Sarvam returned an empty transcript. The recording may be silent.',
    };
    throw err;
  }
  ...
  return {
    text,
    detectedLanguage,
    // Saarika v3 does not return a per-utterance confidence score in the REST API
